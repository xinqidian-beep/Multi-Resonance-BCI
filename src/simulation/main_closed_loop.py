...
def compute_resonance(fear, balance, touch, theta):
    # 1. 中值零化
    f_til = round(fear) - MID['fear']
    b_til = round(balance) - MID['balance']
    t_til = round(touch) - MID['touch']
    th_til = round(theta) - MID['theta']
...
plt.title('Multi-Resonance-BCI Closed-Loop Simulation (±2048 Scaling)')
plt.grid(True)
plt.legend()
...
print("\n✅ 仿真结果图片已保存: simulation_result.png")

# 避免非交互环境报错
try:
    plt.show()
except:
    print("当前为无界面环境，图片已保存。")
