...
# Plot Fear Level
plt.subplot(3, 1, 1)
plt.plot(fears, label='Fear Level', color='red', linewidth=2)
plt.ylabel('Fear Level')
plt.legend()
plt.grid(True)

# Plot Resonance Rate R
plt.subplot(3, 1, 2)
plt.plot(Rs, label='Resonance Rate R', color='blue', linewidth=2)
plt.axhline(y=6, color='orange', linestyle='--', label='Target R=6')
plt.ylabel('R Value')
plt.legend()
plt.grid(True)
...
# Plot GVS Current
plt.subplot(3, 1, 3)
plt.plot(I_gvs_history, label='GVS Current', color='green', linewidth=2)
...
# Save the plot to a file instead of trying to show it (for non-interactive environments)
plt.savefig('bci_simulation_results.png', dpi=300, bbox_inches='tight')
print("图表已保存为 'bci_simulation_results.png'")

# For interactive environments, uncomment the line below:
# plt.show()
